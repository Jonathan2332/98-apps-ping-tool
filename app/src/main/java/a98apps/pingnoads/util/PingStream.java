...
import a98apps.pingnoads.view.TabPing;
import a98apps.pingnoads.view.ViewHolder;

class PingStream extends Thread
{
    ...
    private final TabPing listPing;
    ...
    PingStream(InputStream is, Activity activity, SecurityPreferences p, ViewHolder mViewHolder, TabPing tab, Ping ping)
    {
        setRunning(true);
        this.stream = is;
    ...
    private void resultPing(final String result, final boolean lostPacket)
    {
        activity.runOnUiThread(new Runnable()
        {
            ...
            public void run()
            {
                if(lostPacket)
                {
                    if(totalPacketsLoss == 0)
                        mViewHolder.totalPacketLoss.setText(Html.fromHtml("<font color='#8bc34a'>" + totalPacketsLoss + "</font>"));
                    else
                        mViewHolder.totalPacketLoss.setText(Html.fromHtml("<font color='red'>" + totalPacketsLoss + "</font>"));
                }
                listPing.addItems(result);
            }
        });
    }
    private void initializeCount()
    {
        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mViewHolder.totalPacketLoss.setText("0");
                mViewHolder.totalPacketLoss.setTextColor(activity.getResources().getColor(R.color.colorAccent2));
            }
        });
    }
